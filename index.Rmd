...
image_read('./output/figures/activity.png') %>% 
  ...
  image_write(., path = './output/figures/Fish.png', format = "png")

```

```{r}
n <- data_useful %>% filter(., species == "whitedams" & year == "2016" & treatment == "control") ; nrow(n)
```
```{r}
n <- data_useful %>% filter(., species == "whitedams" & year == "2016" & treatment == "CO2") ; nrow(n)
```