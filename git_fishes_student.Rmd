...
data_group <- data_useful %>% group_by(species, treatment) %>%  
  ...
  mutate(sample_size = length(sl), mean_sl = mean(sl), se_sl = sd(sl) / (sqrt(length(sl))), mean_activity = mean(activity), se_activity = sd(activity) / (sqrt(length(activity))))
...
t_result <- tibble(species = c("1", "1", "1", "1", "1", "1"), mean_diff = 1.1:6.1, CI_min = 1.1:6.1, CI_max = 1.1:6.1, t = 1.1:6.1, df = 1.1:6.1, p = 1.1:6.1)
# Use for loop to calculate results
for (i in seq(1, n_rep, 2)) {
  # Filter data by species and treatment
  data_sort[[i]] <- data_group %>% filter(species == levels(as.factor(data_group$species))[(i + 1) / 2] & treatment ==	"control")
  data_sort[[i + 1]] <- data_group %>% filter(species == levels(as.factor(data_group$species))[(i + 1) / 2] & treatment ==	"CO2")
  # Do t.test
...
# Use flextable to render the summary table in a tidy format
flextable(t_result) %>% add_header_lines(., values = "Table 2: Summary of t test") %>% add_footer_lines(., values = c("mean_diff, mean difference of activity of treatnent and control (treatment - control)", "CI_min, minimum of the 95% confidence interval", "CI_max, maximum of the 95% confidence interval", "t, t-statistic", "df, degree of freedom", "p, p-value"))
