...
```{r loadpacks, message=FALSE, results='hide', eval = F}
...
1. Github provides more record of the detailed history, which you may not want to attention sometimes.
2. Files in Github can be edided by many people, so it is hard to find the lastest file created by you quickly if you have many colleagues.
3. Github relys on internet. You cannot use it when you are offline. You also need to download the files every time you want to edid your file if you use it as a backup.
```
...
data_narm <- data %>% filter(complete.cases(data))
```
...
data_useful <- data_narm[, c(-1, -2, -9)]
# Check spelling in species and treatment but also generate a summary table
table_sp_tr <- as.matrix(table(data_useful$treatment, data_useful$species))
table_sp_tr[, 7] <- c(mean(table_sp_tr[1, 1:6]), mean(table_sp_tr[2, 1:6]))
# Use flextable to render the summary table in a tidy format

flextable(data_useful)
```
