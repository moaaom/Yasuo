...
data_group <- data_useful %>% group_by(species, treatment) %>%  
  ...
  mutate(N = length(activity), mean_activity = mean(activity), se_activity = sd(activity) / (sqrt(length(activity))))
# Delete duplicate data and sort
data_summary <- data_group %>% .[, c(-3)] %>% .[!duplicated(.), ] %>% .[order(.$species, .$treatment), ]
# Use flextable to render the summary table in a tidy format
flextable(data_summary) %>% add_header_lines(., values = "Table 1: Summary of activity data") %>% align(., align = "left") %>% add_footer_lines(., values = c("N, sample size", "mean_activity, mean of activity", "se_activity, standard error of activity")) 
